...
public class UserServiceImpl implements IUserService {
    ...
    public ServerResponse<String> forgetResetPassword(String username,String passwordNew,String forgetToken){
        ...
        if(StringUtils.equals(forgetToken,token)){
            String mdsPassword = MD5Util.MD5EncodeUtf8(passwordNew);
            User user=userMapper.selectLogin(username,mdsPassword);
            if(user!=null){
                return ServerResponse.createByErrorMessage("新旧密码不能一样");
            }
    ...
    public ServerResponse<String> resetPassword(String passwordOld, String passwordNew,User user){
        // 防止横向越权，需要校验用户的旧密码，同时确保是该用户，所以需要制定用户id
    ...
    public ServerResponse<User> updateInformation(User user){
        // username 不可以被更新
        // email也要校验，新的email不能是其他人已绑定的email
        int resultCount=userMapper.checkEmailByUserID(user.getEmail(),user.getId());
        if(resultCount>0){
            return ServerResponse.createByErrorMessage("该邮箱已被使用,请更改邮箱后再尝试更新");
        }
